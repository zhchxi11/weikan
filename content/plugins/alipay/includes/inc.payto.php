...
if( !isset($_REQUEST['nonce']) ) 
	die(ws_alipay_show_tip( 'NONCE_EMPTY' ));
	//wp_verify_nonce( $_REQUEST['nonce'], 'ws_alipay_front_nonce_action')
if( !wp_verify_nonce( $_REQUEST['nonce'], 'fromcart' ) ) 
	die(ws_alipay_show_tip( 'NONCE_INVALID' ));

//############################################################################
//DB SECTION
if( !isset( $_REQUEST['proid'] ) ) die();
//需要的参数, 
$para = array(
'proid','email','msg','extra','addr','tel',
'num','aliacc','referer','postcode','ordname'); 
//把未声明的预使用变量定义为''
$ord = ws_alipay_noEmpty( $para, $_REQUEST );
