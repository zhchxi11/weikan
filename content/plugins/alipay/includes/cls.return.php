...
class wsAlipayReturn{
	...
	function wsAlipayReturn( $para_ret ){
		...
		$this->wpdb 	= $wpdb;
		//创建商品对象
		$this->pro		= new ws_alipay_product();
		//创建订单对象
		$this->order	= new ws_alipay_order();
		//创建设置数组对象
		$this->opt 		= ws_alipay_get_settings();
		//创建邮件对象
		$this->mail 	= new WS_Alipay_Mail();
	...
	function returnProcess(){
		...
		if( $ordtype == 'ADP' ){//ADP 处理
			$this->ad = new WS_Alipay_Ads($this->proid);
			//重算有效时间
			$startTime = $stime;
	...
	private function getSendSrc(){
		...
		//从货源中删除该货源,为数据更新做准备
		$backsrc = ws_alipay_array_reduce( $arr_autosrc, $autosepori );
		//更新数据
