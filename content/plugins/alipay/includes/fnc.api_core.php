 ...
 * return 拼接完成以后的字符串
 */
function ws_alipay_createLinkstring($para) {
	$arg  = "";
 ...
 */
function ws_alipay_paraFilter($para) {
	$para_filter = array();
	while (list ($key, $val) = each ($para)) {
 ...
 * return 排序后的数组
 */
function ws_alipay_argSort($para) {
	ksort($para);
	reset($para);
	return $para;
}
 ...
 */
function ws_alipay_logResult($word='') {
	$fp = fopen("log.txt","a");
	flock($fp, LOCK_EX) ;
 ...
 * return 远程输出的数据
 */
function ws_alipay_getHttpResponse($url, $input_charset = '', $time_out = "60") {
