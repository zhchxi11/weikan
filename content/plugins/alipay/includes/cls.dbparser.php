...
class ws_alipay_db_parser{
	...
	public function __construct( $proid = 0 ){
		...
		if( (int)$proid == 0 ){
			$this->ret = "【提示】:商品参数不合法,未知的商品ID<br/>";
			return;
		}
...
//		$proInfo = $wpdb->get_results( $sql );
		...
		if( empty( $tplinfo[0] ) ){
			$this->ret = '【提示】:该商品未指定模版,请<a href="'.WS_ALIPAY_URL.'/includes/tpl.cart.php?proid='.$proid.'">点此购买</a><br/>';
			return;
		}
		...
		$html1  = '<div class="ws_alipay_buy_wrap">';
		$html1 .= '<input type="hidden" class="ws_alipay_buy_nonce" value="'.$nonce.'"/>';
		$html1 .= '<input type="hidden" class="ws_alipay_buy_protype" value="'.$proInfo['protype'].'"/>';
		$html1 .= '<input type="hidden" class="ws_alipay_buy_fields" value="'.$postFieldStr.'"/>';
		$html1 .= '<input type="hidden" value="[proid]" class="ws_alipay_buy_proid"/>
		' . $html . '
		</div>';
